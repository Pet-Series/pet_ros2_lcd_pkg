# lcddriver.py - Modifyed to be included in a ROS2-package/node.
...
class lcd():
   """ 
   Class to control the 16x2 or 20x4 I2C LCD display wired up to a Raspberry Pi
   class 'input parameter' is the I2C-address as an integer.
   """
   def __init__(self, LCD_I2C_ADDRESS):
      """Setup the display, turn on backlight and text display + ...?"""
      self.device = util.i2c_lib.i2c_device(LCD_I2C_ADDRESS, LCD_I2C_BUS)
      self.write(0x03)
      self.write(0x03)
      self.write(0x03)
      self.write(0x02)
